        ...
        if (isset($_FILES['uavtr'])) {
            ...
            if (isset($uAvtr) &&  $uAvtr != '') {
                if ($size < 5000000) {
                    ...
                    $allowedFormat = ['jpeg', 'jpg', 'png', 'gif', 'jfif'];
...
if (isset($_POST['makeAdmin'])) {
    $sql = "update users_info set is_admin='1' where id='" . $_POST['userID'] . "'";
    $result = mysqli_query($conn, $sql);
   
   
}

if (isset($_POST['deactivateAdmin'])) {
    if($_POST['userID']==$id){
echo "<script>alert('You cant deactivate yourself')</script>";
    }else{
        $sql = "update users_info set is_admin='0' where id='" . $_POST['userID'] . "'";
        $result = mysqli_query($conn, $sql);
    }
                                ...
                                <div class="modal-body">
                                    <form method="POST" action="user.php" enctype="multipart/form-data">
                                        <div class="form-group pt-20">
                                            ...
                                            <div class="col-sm-8">
                                                <input name="uname" type="text" class="login-input border rounded" placeholder="User Name" id="uname" value="<?php echo $thisUser['name']; ?>" required>
                                            </div>
                                        </div>
                                        <div class="form-group pt-20">
                                            <div class="col-sm-4">
                                            ...
                                            <div class="col-sm-8">
                                                <input name="email" type="email" class="login-input rounded" placeholder="Email Address" value="<?php echo $thisUser['email']; ?>" id="email" required>
                                            </div>
                                        </div>
                                        <div class="form-group pt-20">
                                            <div class="col-sm-4">
                ...
                <div class="table_container">
                    ...
                    <div class="table-responsive">
                        <table class="table table-dark" id="table" data-toggle="table" data-search="true" data-filter-control="true" data-show-export="true" data-click-to-select="true" data-toolbar="#toolbar">
                            <thead class="thead-light bg-darks">
                                <tr>
                            ...
                            <tbody>
                                ...
                                if (mysqli_num_rows($res) > 0) {
                                    while ($row = mysqli_fetch_assoc($res)) {
                                        ...
                                        echo '<td style="color:white">' . $active . '</td>';
                                        if ($thisUser['is_admin'] == 1) {
                                ...
                                ?>
                                        ...
                                        <td>
                                            <form method="post" style="display:<?= $row['is_admin'] == 1 ? "block" : "none" ?>">
                                                <input type="hidden" value="<?= $row['id'] ?>" name="userID">
                                                <input type="submit" name="deactivateAdmin" value="Deactivate as admin" class="btn deactivate btn-primary">
                                            </form>
                                            <form method="post" style="display:<?= $row['is_admin'] == 0 ? "block" : "none" ?>">
                                                <input type="hidden" value="<?= $row['id'] ?>" name="userID">
                                                <input type="submit" name="makeAdmin" value="Make Admin" class="btn  btn-primary">
                                            </form>
                                        </td>
                                        <?php
